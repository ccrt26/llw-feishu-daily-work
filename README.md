...
120 秒 AI 分析时限退出，来源保留且零写入。

V4.4.5 候选不再采用“统一改成 300 秒”。程序先按 DOCX 的标准结构确定性提取
...
候选已完成本地 206 项聚焦验证、有效 737 项完整回归和最大资源边界测量。完成生产
部署和同一保留来源的真实重试前，系统总版本仍为 V4.4.3，云文档也仍不宣称为已
验收能力。普通上传文件不受此验收边界影响。
