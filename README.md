...
当前只启用 B 站和抖音两个公开视频门。原生语音、音频文件、直接发送的
本地视频和普通网页仍固定关闭；系统会返回明确的“不支持”，不会假装已理解。
没有安装 FFmpeg、本地 ASR、第三方视频理解模型或通用媒体平台。

飞书云文档的读取、导出和用户 OAuth 权限已经生效。V4.4.4 安全分类热修只允许
严格合法、保持惰性的 HTTP/HTTPS hyperlink；外部图片、模板、OLE、工作簿、
附件、未知关系和混合危险关系仍失败关闭。V4.4.5 在该安全基线上，先按 DOCX 的
标准结构确定性提取
正文、标题、列表、表格、页眉页脚、脚注尾注和内嵌 PNG，并把证据与原件哈希绑定；
这一步由程序完成，但不按文件名、关键词或业务内容猜测，因此不会影响其他格式。
飞书检查/导出和附件下载仍为 120 秒，DOCX 本地准备最多 60 秒；只有含合格 DOCX
且不含公开视频的任务，才让同一个 Personal Assistant 调用一次、最多 10 分钟，
并在 5 分钟时最多提示一次仍在处理。证据不完整仍可明确说明限制，但不能入库。
普通文字、图片、PDF、PPTX、XLSX 和视频仍使用原有 120 秒合同。

V4.4.5 已完成本地 206 项聚焦验证、有效 737 项完整回归和最大资源边界测量。
生产真实验收随后从保留来源生成 123 个文字结构和 14/14 张图片证据，覆盖状态为
`complete`；同一个 Personal Assistant 完成总结和一次知识写入，Outcome 先保存、
最终飞书回复已送达。知识项只生成一份 Markdown 和一份哈希一致的原 DOCX，没有
重复 Writer、暂存或锁残留。该版本没有增加权限、Agent、Router、Writer、模型、
服务、依赖或新的媒体平台。
...
DOCX 同样在 AI 前由短生命周期、无网络的私有子进程读取 OOXML 结构。父进程重新
核对原件、证据索引、正文、每张 PNG 及其所属文档部件中的关系映射，然后才把这些
证据交给同一个 Codex。该 reader 只处理来源检查器已经确认的 DOCX，不接管或猜测
PDF、PPTX、XLSX、图片、文字和视频。
...
- 不自动在模型之间回退；
- 不为当前系统安装 Office、WPS AI、新 AI 插件、FFmpeg 或本地 ASR。
...
V4.4.3 已把 B 站和抖音公开视频作为两个独立门接入同一个 Personal Assistant：
...
B 站标准链接、短链和安全的手机分享链接会先规范化，再进入同一条来源链。每次
用户请求只尝试取得来源一次；网络或站点访问失败、媒体不可用、超限和本地安全
校验失败会返回各自的安全原因，不自动发起第二次请求。用户需要时可以重新发送
同一链接。精确文本“任务结束”或“当前任务已结束”会真正关闭后台 Task Session，
下一条独立视频从新的 `source-001` 任务开始。
