...
V4.4.3 只启用 B 站和抖音两个公开视频门。原生语音、音频文件、直接发送的
本地视频和普通网页仍固定关闭；系统会返回明确的“不支持”，不会假装已理解。
没有安装 FFmpeg、本地 ASR、第三方视频理解模型或通用媒体平台。

飞书云文档的读取、导出和用户 OAuth 权限已经生效。V4.4.4 安全分类热修已经
部署到原有单服务：只允许严格合法、保持惰性的 HTTP/HTTPS hyperlink，外部图片、
模板、OLE、工作簿、附件、未知关系和混合危险关系仍失败关闭。热修没有增加权限、
Agent、Writer 或新主链。真实文档已经通过导出和安全检查，随后在历史固定的
120 秒 AI 分析时限退出，来源保留且零写入。V4.4.5 候选只允许 Personal
Assistant 使用底层既有的 300 秒硬上界；其他三个 AI 时限和所有业务边界不变。
完成生产部署和同一保留来源的真实重试前，系统总版本仍为 V4.4.3，云文档也仍不
宣称为已验收能力。普通上传文件不受此验收边界影响。
...
- 不自动在模型之间回退；
- 不为 V4.4.3 安装 Office、WPS AI、新 AI 插件、FFmpeg 或本地 ASR。
