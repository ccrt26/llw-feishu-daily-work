...
V4.2.0 Phase 0 只补齐共用视觉证据基础。原生语音、音频文件、本地视频、
网页、Bilibili 和抖音六个生产开关仍固定为关闭；系统会返回明确的“不支持”
结果，不会假装已理解，也没有安装转写或视频处理运行件。
...
- 不自动在模型之间回退；
- 不为 V4.2.0 Phase 0 安装 Office、WPS AI、新 AI 插件、音频转写或视频处理软件。
...
V4.2.5 已把 B 站公开视频作为独立门接入同一个 Personal Assistant：

```text
用户输入 B 站链接
→ 匿名固定 view/playurl 合同取得完整 M4A 和 MP4
→ 火山录音文件识别 1.0 返回带时间戳文字
→ AVFoundation/Core Media 生成覆盖开始到结尾的联系表
→ 同一个 Codex 结合声音和真实画面直接回复
```

生产只打开 `bilibiliEnabled`；语音消息、音频文件、直接视频文件、普通网页和
抖音门仍关闭。没有增加 Router、Agent、第五个副作用工具、第三方视频理解模型、
FFmpeg、本地 ASR 或自动模型回退。火山凭据只从 macOS 钥匙串读取，19 小时
试用硬上限由失败关闭的本地账本控制。

视频时间线 helper 使用已审计的 Apple 系统框架，固定工件 SHA-256 为
`b3b79f1770b49b75223d4a085ba41001256c985a3bde36d3317b9dd90a8f5a3f`。
共享核心完整回归为 842/842，精确生产暂存包聚焦合同和微信形态纵向旅程为
90/90。真实公开样例 `BV1Y3Ne6hENH` 的部署前无 ASR 读取取得完整
