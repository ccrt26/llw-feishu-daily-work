...
Production deployment: **performed and accepted through WeChat**

## Release position

The continuous Task Session implementation has passed deterministic end-to-end journeys, the complete component regression, one real isolated Codex smoke and one owner-originated WeChat production journey. The component plus Skill were switched together and the production behavior is accepted.

Media remains disabled under `STOP_AFTER_FOUNDATION`. No media runtime, new permission, extra process, router, registry or side-effect tool was added. The V4.1.0 deployment changed only the verified component, Skill manifest hash and safe Task Session state migration.

The production Skill working branch briefly contained the 4.1.0 commit during local verification. Read-only production inspection detected that this path is also the live Skill source, so the new contract could not safely remain beside the old component. Commit `4a9ed70` was preserved on the isolated implementation branch, and production Skill content was temporarily restored to 4.0.1 with auditable revert commit `4a5d8db`. The final deployment later switched the verified component and exact `4a9ed70` Skill together while the service was stopped.
...
## Atomic deployment evidence

Immediately before the switch, a second protected live-state snapshot preserved the current config, state, heartbeat, model state, WeChat state and LaunchAgent plist. The directory is `0700`, every artifact is `0600`, and its SHA-256 and mode manifests passed.

With the single LaunchAgent unloaded and no remaining consumer process:

- the production component switched to `9c0b46babf0c78a1e0e8bddf4aeba1917691b132` on `production/v410-continuous-tasks`;
- the production Skill repository switched to the exact verified `4a9ed709e50a0edae28692b6d174b2e1f2841184` on its corresponding production branch;
- the version-6 config atomically changed only its expected Skill manifest SHA-256;
- the same LaunchAgent restarted with one main process and one direct Feishu event consumer;
- heartbeat was valid, no startup error signature appeared, and media remained disabled;
- the prior WeChat conversation became one active WeChat Task Session with all six bounded turns and its waiting state preserved;
- Feishu remained empty and isolated.

The production-directory focused suite then passed:
...
## Real WeChat production acceptance

The project owner requires real channel tests to prefer WeChat. The owner explicitly ended the migrated prior task, then sent the generated non-private TXT by itself. The assistant asked its purpose; the owner later requested a summary while explicitly forbidding saving or file generation.

The redacted production facts are:

- a fresh WeChat Task Session remained active at revision 2 with resolved revision 2 and no pending input;
- the task retained exactly one source, and its SHA-256 exactly matched the generated fixture;
- the task source manifest and state source IDs matched; retained bytes were mode `0600` in a mode-`0700` task directory;
- the four recent task turns were `user → assistant → user → assistant`;
- the purpose-question Outcome and final committed Outcome were both durable and replied through WeChat;
- the final Outcome had zero artifacts and zero reply files, no Writer checkpoint remained, and the synthetic fixture text did not exist anywhere in the workspace knowledge libraries;
- Feishu had no current Task Session and was not mutated.

This is the requested user-visible chain:

```text
real WeChat file-only input
→ purpose question
→ later instruction
→ same retained source
→ direct summary
→ zero Writer
→ durable Outcome
→ WeChat reply
