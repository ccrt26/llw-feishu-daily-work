# V4.4.5 DOCX 证据准备与十分钟分析发布报告

日期：2026-08-02

状态：已实现、部署并完成真实飞书纵向验收；生产总版本为 V4.4.5。
...
- 实现提交：
  ...
  - `65da141`：精确支持真实飞书导出的标准 XML、元数据、VML 图片和表格合同。
...
第一次生产重试在 DOCX 准备阶段失败关闭，未调用 AI 或 Writer。主智能体对保留
原件做只读、脱敏结构诊断后确认四项根因：预定义 `xml` 前缀被误判为未绑定；
页眉使用标准 VML 图片；package 根部存在标准 core/app 元数据关系；编号表格
单元格携带了与下游 observation 合同冲突的 `level`。每项先加入失败测试，再做
最小修复；纯矢量 VML、混合额外矢量、非法命名空间和未知关系继续失败或降级为
`partial`，没有把“HTTP/HTTPS”或 VML 整体放宽。

最后修复后的受影响合同为 125/125，通过；正式 helper 直接合同为 18/18，通过。
同一保留来源的生产只读准备结果为：123 个文字结构、14/14 张图片、覆盖
`complete`，发布 18 个 owner-only 证据文件且 job 清理完成。最终飞书任务形成
`committed` Outcome 并已回复；Writer 只建立一个知识项，其中恰好一份 Markdown
和一份原 DOCX，DOCX 哈希与保留来源一致，无重复写入、锁、stage 或 job 残留。

生产仍是一个 LaunchAgent、一个 Node 主进程和一个直属飞书消费者；配置 schema
仍为 7，默认 AI 时限仍为 120,000 ms。此次发布没有新增权限、Agent、Router、
Writer、模型、服务、依赖、媒体平台或知识格式。
...
V4.4.5 已通过发布门，配置保持 `personalAssistant.aiTimeoutMs=120000`。部署前
候选和最后标准结构热修均分别建立 owner-only 回滚点并从全新临时目录验证内容
与哈希；回滚包不进入 Git，也不包含正文、平台标识、令牌或知识库资料。V4.4.4
仍是安全前置基线，十分钟上限不会因单个文档处理困难而继续提高；超过十分钟仍
保留来源、零误写并要求把任务拆小。
